...
# Realizado em 16 e 17/05/2022 
# Pacote shiny (app para publicar o resultado)

# Configurando o diretório de trabalho

setwd("C:/Users/deeww/OneDrive/Área de Trabalho/Ciencia de dados/Curso Formação Cientista de dados/BigData_R_Azure_ML/cap04")
getwd()
...
# nome da coluna: Cost.Per.Month...Standard.... - dados_netflix
...
#v64 é uma coluna; o conjunto de dados foi chamado de world bank
...
# Combina os dataframes (agrupa o dados_salario com netflix_pib2020)
...
# Limpa e filtra o dataframe IMDB (slicing) todas as colunas, menos essas colunas (-c)
# Modifica o nome da coluna de primarytitle para show_title, para poder juntar
...
# Mantém apenas informação de gênero de filme por país (slicing)
topgeneropaises <- topgenero[,-c(1, 3:9)]
View(topgeneropaises)

# Pivot do dataframe (transforma linha em coluna)
...
# Conta o número de gêneros (omit remove os valores ausentes)
...
# Calcula soma (total de genêros por países)
total = sum(added$n)
total
...
top10add <- rename(top10add, id = Group.1)   #renomeia
top10add <- rename(top10add, n = x)
top10add$label = sub("total  -  ", "", top10add$id) #filtra e tira o "-"
top10add$parent = c("total")
top10add$n <- as.numeric(top10add$n)
total = sum(top10add$n)    #cálculo do total
top10sunburst <- rbind(top10add, top10sunburst)   #junção por linha
top10sunburst <- rbind(c("total", total, NA, NA, "total"), top10sunburst)
top10sunburst$n <- as.numeric(top10sunburst$n)  #Transforma em numeric
View(top10sunburst)
...
# Filtra o dataframe anterior e cria um novo (árvore de países - Mapa)
